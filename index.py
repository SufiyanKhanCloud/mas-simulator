...
def mm1_simulation(lambda_val, mu_val, use_priority):
    # Safety parameters
    threshold = 0.99999
    max_k = 500  
    pmf = []
    ...
    # NEW: Generate Priorities (1=High, 4=Low)
    priorities = [np.random.randint(1, 4) for _ in range(len(arrival_times))]
    
    # Fixed the zero service time issue with math.ceil
    service_times = [max(1, math.ceil(-mu_val * math.log(np.random.rand()))) for _ in range(len(arrival_times))]
    
    service_starts = [0] * len(arrival_times)
    service_ends = [0] * len(arrival_times)

    # --- Priority Logic for M/M/1 ---
    if use_priority:
        served_mask = [False] * len(arrival_times)
        current_time = 0
        for _ in range(len(arrival_times)):
            # Find customers who have arrived but not served
            candidates = [i for i in range(len(arrival_times)) if not served_mask[i] and arrival_times[i] <= current_time]
            
            if not candidates:
                # If no one arrived, pick the absolute next one in time
            ...
            service_starts[chosen_idx] = max(arrival_times[chosen_idx], current_time)
            service_ends[chosen_idx] = service_starts[chosen_idx] + service_times[chosen_idx]
            current_time = service_ends[chosen_idx]
    ...
    df = pd.DataFrame({
        "Observation": obs_numbers,
        "Priority": priorities, # New Column
        "Inter-arrivals": inter_arrivals,
        "Arrival Times": arrival_times.astype(int),
        "Service Time": service_times,
        "Service Start": service_starts,
...
def mms_simulation(lambda_val, mu_val, servers, use_priority):
    threshold = 0.99999
    max_k = 500  
    pmf = []
    cp_cum = []
    ...
    while True:
        ...
        cp_cum.append(round(cum, 5))
        no_between_arrivals.append(k)
        if cum >= threshold or k >= max_k: break
        k += 1
        current_p = current_p * (lambda_val / k)

    if cp_cum: cp_cum[-1] = min(cp_cum[-1], 1.0)
    cp_lookup = [0.0] + cp_cum[:-1]
    ...
    for _ in range(1, n_ranges):
        ...
        for i in range(n_ranges):
            if cp_lookup[i] <= r <= cp_cum[i]:
                inter_arrivals.append(i); found = True; break
        if not found: inter_arrivals.append(n_ranges - 1)

    arrival_times = np.cumsum(inter_arrivals)
    obs_numbers = list(range(1, len(arrival_times) + 1))
    
    # NEW: Generate Priorities
    priorities = [np.random.randint(1, 4) for _ in range(len(arrival_times))]
    service_times = [max(1, math.ceil(-mu_val * math.log(np.random.rand()))) for _ in range(len(arrival_times))]
    ...
    # --- Priority Logic for M/M/S ---
    for _ in range(len(arrival_times)):
        next_free_time = min(server_end_times)
        
        # Check who is waiting
        candidates = [i for i in range(len(arrival_times)) if not served_mask[i] and arrival_times[i] <= next_free_time]
        
        if not candidates:
            chosen_idx = next(i for i, s in enumerate(served_mask) if not s)
        else:
            if use_priority:
                chosen_idx = min(candidates, key=lambda x: priorities[x])
            else:
        ...
        # Prioritize Server 1 if both idle
    ...
    df = pd.DataFrame({
        ...
        "Service End": service_end,
        "Server": server_assigned,
        "Wait Time": [service_start[i] - arrival_times[i] for i in range(len(arrival_times))]
    })
    return df
...
def show_table(parent_frame, df, lambda_val, mu_val, servers):
    ...
    btn_back = tk.Button(scrollable_frame, text="← Back", bg="#888844", fg="white", command=lambda: raise_frame(main_frame))
    btn_back.pack(anchor="w", padx=10, pady=10)

    table_frame = tk.Frame(scrollable_frame, bg="#666633")
    table_frame.pack(pady=20)
    
    for j, col in enumerate(df.columns):
        tk.Label(table_frame, text=col, font=("Arial", 10, "bold"), bg="#999966", fg="white", padx=5).grid(row=0, column=j, sticky="nsew")

    for i, row in enumerate(df.itertuples(index=False), start=1):
        for j, val in enumerate(row):
            tk.Label(table_frame, text=val, bg="#aaa977" if i%2==0 else "#999966", fg="white").grid(row=i, column=j, sticky="nsew")

    if servers > 1: draw_mms_gantt(df, scrollable_frame)
    else: draw_mm1_gantt(df, scrollable_frame)

def draw_mm1_gantt(df, scrollable_frame):
    fig, ax = plt.subplots(figsize=(10, 2), facecolor="#666633")
    ax.set_facecolor("#666633")
    x = 0
    for i in range(len(df)):
        ax.add_patch(plt.Rectangle((x, 0), 1.5, 1, facecolor="#A56A64", edgecolor="white"))
        ax.text(x+0.75, 0.5, f"C{df.iloc[i]['Observation']}", color="white", ha="center")
        x += 1.5
    ax.set_xlim(0, x); ax.axis('off')
    FigureCanvasTkAgg(fig, master=scrollable_frame).get_tk_widget().pack()
...
def draw_mms_gantt(df, scrollable_frame):
    for srv in df['Server'].unique():
        ...
        x = 0
        for i in range(len(srv_df)):
            ax.add_patch(plt.Rectangle((x, 0), 1.5, 1, facecolor="#7D719B", edgecolor="white"))
            ax.text(x+0.75, 0.5, f"C{srv_df.iloc[i]['Observation']}", color="white", ha="center")
            x += 1.5
        ax.set_title(f"{srv} Timeline", color="white")
        ax.set_xlim(0, x); ax.axis('off')
        FigureCanvasTkAgg(fig, master=scrollable_frame).get_tk_widget().pack()

def mm1_input_page():
    raise_frame(mm1_frame)
    for w in mm1_frame.winfo_children(): w.destroy()
    c = tk.Frame(mm1_frame, bg="#666633"); c.pack(expand=True)
    tk.Label(c, text="M/M/1 + Priority", font=("Arial", 20), bg="#666633", fg="white").pack(pady=20)
    
    l_entry = tk.Entry(c, justify="center"); l_entry.pack(pady=5); l_entry.insert(0, "Lambda")
    m_entry = tk.Entry(c, justify="center"); m_entry.pack(pady=5); m_entry.insert(0, "Mu")
    
    p_var = tk.BooleanVar()
    tk.Checkbutton(c, text="Enable Priority", variable=p_var, bg="#666633", fg="white", selectcolor="#333311").pack()
    
    tk.Button(c, text="Simulate", command=lambda: show_table(mm1_frame, mm1_simulation(float(l_entry.get()), float(m_entry.get()), p_var.get()), float(l_entry.get()), float(m_entry.get()), 1)).pack(pady=10)
    tk.Button(c, text="Back", command=lambda: raise_frame(main_frame)).pack()

def mms_input_page():
    raise_frame(mms_frame)
    for w in mms_frame.winfo_children(): w.destroy()
    c = tk.Frame(mms_frame, bg="#666633"); c.pack(expand=True)
    tk.Label(c, text="M/M/S + Priority", font=("Arial", 20), bg="#666633", fg="white").pack(pady=20)
    
    l_entry = tk.Entry(c); l_entry.pack(pady=5); l_entry.insert(0, "5.23")
    m_entry = tk.Entry(c); m_entry.pack(pady=5); m_entry.insert(0, "6.95")
    s_entry = tk.Entry(c); s_entry.pack(pady=5); s_entry.insert(0, "2")
    
    p_var = tk.BooleanVar()
    tk.Checkbutton(c, text="Enable Priority", variable=p_var, bg="#666633", fg="white", selectcolor="#333311").pack()
    
    tk.Button(c, text="Simulate", command=lambda: show_table(mms_frame, mms_simulation(float(l_entry.get()), float(m_entry.get()), int(s_entry.get()), p_var.get()), float(l_entry.get()), float(m_entry.get()), int(s_entry.get()))).pack(pady=10)
    tk.Button(c, text="Back", command=lambda: raise_frame(main_frame)).pack()

def raise_frame(frame): frame.tkraise()

# --- Main Window ---
root = tk.Tk()
root.attributes('-fullscreen', True)
main_frame = tk.Frame(root, bg="#666633")
mm1_frame = tk.Frame(root, bg="#666633")
mms_frame = tk.Frame(root, bg="#666633")

for f in (main_frame, mm1_frame, mms_frame): f.place(relwidth=1, relheight=1)

mc = tk.Frame(main_frame, bg="#666633"); mc.pack(expand=True)
tk.Label(mc, text="QUEUE SIMULATOR", font=("Arial", 25, "bold"), bg="#666633", fg="white").pack(pady=40)

tk.Button(mc, text="M/M/1", font=("Arial", 15), bg="#888844", fg="white", command=mm1_input_page, width=20).pack(pady=10)
tk.Button(mc, text="M/M/S", font=("Arial", 15), bg="#888844", fg="white", command=mms_input_page, width=20).pack(pady=10)

# EXIT BUTTON
tk.Button(mc, text="QUIT", font=("Arial", 12), bg="#494920", fg="white", command=root.destroy, width=10).pack(pady=30)
